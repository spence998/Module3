...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # all the support images from a given class
            prototypes = []
            ...
            # Calculating the distance to the prototypes & using it to make preds
            query_emb = torch.stack(
                [query_emb] * labels_support.unique().shape[0], dim=1
            )
            query_prototypes = torch.stack(
                [prototypes] * query_emb.shape[0]
            )
            query_preds = torch.norm(query_prototypes - query_emb, dim=-1).square()

            support_emb = torch.stack(
                [support_emb] * labels_support.unique().shape[0], dim=1
            )
            support_prototypes = torch.stack(
                [prototypes] * support_emb.shape[0]
            )
            support_preds = torch.norm(support_prototypes - support_emb, dim=-1).square()

            # Calculate the cross entropy loss for the labels against
            # the pred for the query set
            # Calculate the accuracy for query and support set
            loss = F.cross_entropy(-query_preds, labels_query)
            support_acc = util.score(-support_preds, labels_support)
            query_acc = util.score(-query_preds, labels_query)
